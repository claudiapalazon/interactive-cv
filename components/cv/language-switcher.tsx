...
export function LanguageSwitcher() {
  ...
  return (
    <div className="flex items-center gap-2 sm:gap-3 print:hidden">
      <div className="flex rounded-full bg-black/5 p-0.5">
        {(["es", "en"] as const).map((lang) => (
          <button
            ...
            className={`px-2.5 sm:px-3 py-1 sm:py-1.5 text-[11px] sm:text-[12px] font-semibold rounded-full transition-colors ${
              locale === lang
                ? "bg-black text-white"
      ...
      <div className="h-3 sm:h-4 w-px bg-black/15" aria-hidden />
      <div className="flex gap-0.5 sm:gap-1">
        <button
          type="button"
          onClick={() => setLocaleForPrint("es")}
          className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-1 sm:py-1.5 text-[10px] sm:text-[11px] font-medium text-black/70 hover:text-black hover:bg-black/5 rounded-md transition-colors"
